...
{
FILE *INIFile;
char *startpath, drive[3], dummy[70];

  if ((INIFile=openfile("fx.ini"))!=NULL)
  {
    ...
    startpath=getdata(INIFile,"StartUpDirectory");
    chdir(startpath);
...
{
test=LoadS3MF(file_to_load);  //Lade S3M Modul und reserviere Speicher
if (test!=0)
    {
    OldScreen();
    ClosePlay();
    ...
    if (test==1) printf("%s is not a usual S3M File !\n",FileName);
...
int run_s3m(void)
{
   ...
   if (check == 255) { ClosePlay(); return 0; }   // Fehler aufgetreten
