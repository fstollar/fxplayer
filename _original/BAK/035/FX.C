...
unsigned char S3MHeader[0x65];
char far drives[26][27];
int anzlw, lfw_neu, lfw_start;
char far fname[160][27];
...
int akt_drive;
char far pfad[100];
char far startpath[200],far aktpath[200];
int alength=200;
int status;
char ext[15];
char dest[20], *dest2;
...
extern far unsigned int  InitSB (int BasisIO, char IRQ, char DMA , unsigned int MixSpeed, char StereoFlag);
