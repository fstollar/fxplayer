...
char *endptr;
  ...
  if ((INIFile=fopen("fx.ini","rt"))!=NULL)
  {
    ...
    if ((strcmpi(dummy,"Stereo"))==0) SBStereo=1;
    if ((strcmpi(dummy,"WideMono"))==0) SBStereo=3;
    if ((strcmpi(dummy,"WideStereo"))==0) SBStereo=0;
...
int sb_check(void)
{
test=InitSB (SBAdr,SBIRQ,SBDMA,MixSpeed,SBStereo);
if (test==0xffff)
   {
